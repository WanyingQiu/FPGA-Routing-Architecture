...
def process_benchmark_file(file_path, seeds, lengths):
    benchmark_results = []
    for length in lengths:
        xml_path = f"k6_N10_40nm_length_{length}.xml"
        ave_channel_width = 0
...
def main():
    ...
    output_file = os.path.join(results_folder, 'vpr_results.txt')

    with open(output_file, 'w') as file:
        file.write("File Name, Length, Avg Channel Width, Avg Routing Area, Avg Per Logic Tile, Avg Critical Path Delay\n")

    # Run each benchmark file in parallel
    with ProcessPoolExecutor() as executor:
        future_to_file = {executor.submit(process_benchmark_file, file_path, seeds, lengths): file_path for file_path in files}
