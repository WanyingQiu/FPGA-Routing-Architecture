...
def run_vpr(seed, file_path, xml_path):
    command = f"./vpr {xml_path} {file_path} --seed {seed} --flat_routing true --max_router_iterations 200"
...
def run_vpr_with_channel_width(seed, file_path, xml_path, channel_width):
    ...
    command = f"./vpr {xml_path} {file_path} --seed {seed} --flat_routing true --max_router_iterations 200 --route_chan_width {low_stress_width}"
    print(f"Running: {command}")
...
def process_benchmark_file(file_path, seeds, vals):
    ...
    for val in vals:
        ...
        xml_path = f"k6_N10_sparse_crossbar_40nm.xml"
...
def main():
    ...
    output_file = os.path.join(results_folder, 'vpr_results_b2.txt')
